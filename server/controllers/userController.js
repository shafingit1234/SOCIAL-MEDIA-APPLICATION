...
const deleteMyProfile = async (req, res) => {
  try {
    ...
    //removed myself from followers following list....
    ...
    //removing myself from following's followers list....
    ...
    return res.send(success(200, "User successfully deleted!!!"));
  } catch (e) {
    console.log(e);
    return res.send(error(500, e.message));
  }
...
const getMyInfo = async (req, res) => {
  //difference between findById (logn) and find (On)
  try {
...
const updateUserProfile = async (req, res) => {
  try {
    ...
    if (userImg) {
      //converting base64 to img. using npm i cloudinary
      //go to cloudinary, sign in store keys in env file.
      // user.avatar = userImg;
...
const getUserProfile = async (req, res) => {
  try {
    //to map we create a util names mapostoutput.
    const userId = req.body.userId;
